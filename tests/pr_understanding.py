...
def build_file_graph(cloned_repo: ClonedRepo, k=400, sweep_config: SweepConfig = None):
    ...
    for idx, commit_stats_files in tqdm(enumerate(git_commit_files)):
        files = [
            file
            for file in commit_stats_files.keys()
            if commit_stats_files[file]["insertions"] > 10
            and filter_file(
                f"{cloned_repo.repo_dir}/",
                f"{cloned_repo.repo_dir}/{file}",
                sweep_config,
            )
        ]
    ...
    for files, file_scores in zip(all_files, all_file_scores):
        for i in range(len(files)):
            for j in range(i + 1, len(files)):
                ...
                idf = log(
                    total_commits
                    / (file_count[files[i]] + DAMPENING)
                    * log(total_commits / file_count[files[j]] + DAMPENING)
                )
                ...
                file_score_i = min(
                    file_scores[files[i]] / all_file_lengths[files[i]], 0.2
                ) * log(all_file_lengths[files[i]])
                file_score_j = min(
                    file_scores[files[j]] / all_file_lengths[files[j]], 0.2
                ) * log(all_file_lengths[files[j]])
                weight = tf * idf * file_score_i * file_score_j
                logger.info(
                    f"Adding edge between {files[i]} and {files[j]} with weight {weight}, tf: {tf}, idf: {idf}, file_score_i: {file_score_i}, file_score_j: {file_score_j}"
                )
...
def get_relevant_neighbors(G: Graph, candidate_node, top_k=10):
    sorted_neighbors = sorted(
        [i for i in dict(file_graph[candidate_file]).items()],
