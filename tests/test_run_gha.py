...
def run_gha_on_branch(
    run_name, original_gha_file, original_branch_name, command, repo: Repository
) -> int:
    # Create a new branch
    new_branch_name = f"{original_branch_name}-run"
    # get last page of branches
    existing_branches = [
        branch.name for branch in repo.get_branches()[:40]
    ]  # only get the last 40 branches
    for _ in range(1, 10):
        ...
        if new_branch_name not in existing_branches:
            try:
                repo.create_git_ref(
                    ...
                    sha=repo.get_branch(original_branch_name).commit.sha,
                )
                break
            except GithubException as e:
                if e.status == 422:  # Branch already exists
                    print(
                        f"Branch {new_branch_name} already exists. Skipping branch creation."
                    )
                else:
                    raise e
    ...
    line_with_placeholder = [
        line for line in gha_content.split("\n") if 'echo "Placeholder"' in line
    ][0]
