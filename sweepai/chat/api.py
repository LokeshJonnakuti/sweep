...
def get_cloned_repo(
    ...
    if branch:
        ...
        try:
            ...
            logger.warning(
                f"Error checking out branch {branch}: {e}. Trying to checkout PRs."
            )
        ...
        cloned_repo = MockClonedRepo(
            f"{repo_cache}/{repo}", repo_name, token=access_token
        )
...
def get_authenticated_github_client(repo_name: str, access_token: str):
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                f"Error getting installation for {repo_name}: {e}. Double-check if the app is installed for this repo."
            )
        try:
            repo = g.get_repo(repo_name)
        except Exception as e:
        ...
        if repo.has_in_collaborators(user.login):
            ...
            raise Exception(
                f"User {user.login} does not have the necessary permissions for the repository {repo_name}."
            )
...
def check_repo_exists(
    ...
    try:
        ...
        git.Repo.clone_from(
            f"https://x-access-token:{access_token}@github.com/{repo_name}",
            f"{repo_cache}/{repo}",
        )
...
async def autofix(
    ...
    cloned_repo = ClonedRepo(
        repo_name, installation_id=installation_id, token=access_token, branch=branch
    )
