...
def add_slack_context(summary) -> str:
    ...
    if not SLACK_API_KEY:
        return result
    slack_link_match = re.search(
        r"(https://[\w-]+\.slack\.com/archives/\w+/p\d+)", summary
    )
    ...
    try:
        ...
        thread_messages = get_thread_by_thread_ts(
            client=slack_client, channel_id=slack_channel_id, thread_ts=slack_message_ts
        )
        ...
        for idx, thread_message in enumerate(thread_messages):
            result += (
                f"Message {idx}: {thread_message['text']}\n"
