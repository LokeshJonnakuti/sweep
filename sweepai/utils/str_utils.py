...
def object_to_xml(object: object, object_name: str, exclude_fields: list[str] = []):
    object_fields = [
...
def objects_to_xml(
    ...
    for object in objects:
        objects_string += (
            f"{object_to_xml(object, object_name, exclude_fields=exclude_fields)}\n"
        )
    if outer_field_name:
...
def extract_xml_tag(string: str, tag: str, include_closing_tag: bool = True):
    pattern = (
        f"<{tag}>(.*?)</{tag}>" if include_closing_tag else f"<{tag}>(.*?)(\Z|</{tag}>)"
    )
...
def extract_objects_from_string(
    text: str, object_tag: str, object_params: list[str] = []
):
    extracted_objects = []
    ...
    for match in object_matches:
        ...
        if not failed:
            ...
            logger.warning(
                f"Failure occured during extraction on the following param: {failed_param}\ntext:\n{text}"
            )
            posthog.capture(
