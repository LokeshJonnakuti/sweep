...
class DirectoryTree:
    def __init__(self):
        ...
        self.relevant_files: list[
            str
        ] = []  # this is for __str__ method if the resulting string becomes too large
    ...
    def __str__(self):
        ...
        if len(raw_str) > 20000:
            results = []
            logger.warning(
                f"While attempting to dump the directory tree, the string was too large: {len(raw_str)}. Outputting the truncated version instead..."
            )
            for line in self.lines:
                # always print out directories
                if line.is_dir:
                    line_text = (
                        line.text.split("/")[-2] + "/" if line.is_dir else line.text
                    )
                    results.append(("  " * line.indent_count) + line_text)
                    continue
                ...
                if full_path_of_file in self.relevant_files:
                    ...
                    len(results) > 0
                    and results[-1] != ("  " * line.indent_count) + "..."
                ):
