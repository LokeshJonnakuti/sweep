...
def separate_snippets_by_type(snippets: list[Snippet]) -> SeparatedSnippets:
    ...
    for snippet in snippets:
        for type_name, separation in code_snippet_separation_features.items():
            if (
                any(
                    snippet.file_path.startswith(prefix)
                    for prefix in separation["prefix"]
                )
                or any(
                    snippet.file_path.endswith(suffix)
                    for suffix in separation["suffix"]
                )
                or any(
                    substring in snippet.file_path
                    for substring in separation["substring"]
                )
            ):
    ...
    with Timer() as timer:
        ...
        with Timer() as timer:
            content_to_lexical_score_list = [
                search_index(query, lexical_index) for query in queries
            ]
    ...
    for i, query in enumerate(queries):
        for snippet in tqdm(snippets):
            ...
            if snippet.denotation in content_to_lexical_score_list[i]:
                ...
                snippet_score = (
                    content_to_lexical_score_list[i][snippet.denotation]
            ...
            content_to_lexical_score_list[i][
                snippet.denotation
    ...
    **kwargs,
    ...
    for (
        ...
        **kwargs,
        ...
        yield message, ranked_snippets_list[
            0
        ] if ranked_snippets_list else [], snippets, content_to_lexical_score_list[
            0
        ] if content_to_lexical_score_list else []
...
def get_pointwise_reranked_snippet_scores(
    ...
    new_snippet_scores = {
        snippet_denotation: v / 1_000_000_000_000
    ...
    for snippet in sorted_snippets[:NUM_SNIPPETS_TO_KEEP]:
        new_snippet_scores[snippet.denotation] = (
            rerank_scores[snippet.denotation] * 1_000
        )
...
def multi_prep_snippets(
    ...
    if len(queries) > 1:
        ...
        for (
            ...
            snippets,
            content_to_lexical_score,
        ) in get_top_k_snippets.stream(cloned_repo, queries[0], k):
    ...
    if not skip_pointwise_reranking:
        ...
        for type_name, snippets_subset in separated_snippets:
            new_content_to_lexical_scores = new_content_to_lexical_score_by_type[
                type_name
            ]
            ...
            separated_snippets.override_list(
                attribute_name=type_name, new_list=snippets_subset
            )
            ...
            for idx, snippet in enumerate(snippets_subset[:max_results]):
                ...
                if (
                    percentile < type_to_percentile_floor[type_name]
                    or snippet.score < type_to_score_floor[type_name]
                ):
                    break
            ...
            if (
                type_name != "source"
                and filtered_subset_snippets
                and not skip_analyze_agent
            ):  # do more filtering
                filtered_subset_snippets = AnalyzeSnippetAgent().analyze_snippets(
                    filtered_subset_snippets, type_name, queries[0]
                )
            logger.info(
                f"Length of filtered subset snippets for {type_name}: {len(filtered_subset_snippets)}"
            )
    ...
    if not skip_reranking and skip_pointwise_reranking:
        ranked_snippets[:NUM_SNIPPETS_TO_RERANK] = listwise_rerank_snippets(
            queries[0], ranked_snippets[:NUM_SNIPPETS_TO_RERANK]
        )
...
def get_relevant_context(
    ...
    if (
        not repo_context_manager.current_top_snippets
        and not repo_context_manager.read_only_snippets
    ):
        repo_context_manager.current_top_snippets = copy.deepcopy(previous_top_snippets)
        repo_context_manager.read_only_snippets = copy.deepcopy(
            previous_read_only_snippets
        )
...
def fetch_relevant_files(
    ...
    try:
        ...
        for message, ranked_snippets in prep_snippets.stream(
            cloned_repo, search_query, ticket_progress
        ):
            repo_context_manager = RepoContextManager(
