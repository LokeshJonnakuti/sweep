...
def get_image_urls_from_issue(num: int, repo_full_name: str, installation_id: int):
    ...
    try:
        ...
        for match in image_url_matches:
            ...
            if not added:
                logger.warning(
                    f"Did not add image url: {url}\nReason: image type unsupported!"
                )
    except Exception as e:
        logger.error(
            f"Encountered error while attempting to fetch raw issue {num} for {repo_full_name}:\n{e}"
        )
    return urls
...
def match_image_url_to_image_contents(
    image_urls: dict[str, str], image_contents: dict[str, dict[str, str]]
):
...
def create_message_with_images(
    message: dict[str, str], images: dict[str, dict[str, str]], use_openai: bool = False
):
    if not images:
        return message
