...
@file_cache()
def get_pr_changes(
    repo: Repository, pr: PullRequest, cloned_repo: ClonedRepo
...
def format_patches_for_pr_change(
    pr_change: PRChange, include_patch_annotations: bool = True
):
    patches = ""
    ...
    formatted_patches = patches_for_file_format.format(
        file_name=pr_change.file_name, file_patches=patches
    )
...
def format_all_pr_changes_by_groups(
    ...
    for _, files_to_review in grouped_files.items():
        ...
        changes_to_review = {
            file_name: pr_changes[file_name] for file_name in files_to_review
        }
        ...
        if formatted_changes:
            ...
            for file_name in files_to_review:
                ...
                ) = render_pr_changes(changes_to_review)
                group = GroupedFilesForReview(
...
class PRReviewBot(ChatGPT):
    ...
    def get_special_rules(
        ...
        for file_name in file_names:
            ...
            if not os.path.exists(full_path) or base_file == special_rule_file:
                logger.error(
                    f"Failure fetching special rules file for {file_name} as it does not exist."
                )
            ...
            for i in range(len(subdirectories) - 1, 0, -1):
                directories_to_check.append(
                    os.path.join(
                        cloned_repo.repo_dir, os.path.join(*subdirectories[:i])
                    )
                )
                ...
                if os.path.exists(special_rule_path):
                    if special_rule_path not in added_rules:
                        ...
                        special_rules[file_name].append(
                            read_file_with_fallback_encodings(special_rule_path)
                        )
    ...
    def review_code_changes_by_file(
        ...
        for group_name, grouped_files in formatted_pr_changes_by_group.items():
            ...
            if special_rules:
                ...
                for file_name, special_rule_set in special_rules.items():
                    for special_rule in special_rule_set:
                        rendered_file_changes = formatted_pr_changes_by_file[
                            file_name
                        ].rendered_changes
                        formatted_user_prompt_special_rules = (
            ...
            formatted_user_prompt_edge_case_question_creation = (
            ...
            if not failed:
                ...
                formatted_user_prompt_edge_case_answer = (
    ...
    def identify_functions_in_patches(
        self, pr_changes: dict[str, PRChange], chat_logger: ChatLogger | None = None
    ):
    ...
    def sort_code_issues_by_severity(
        ...
        formatted_user_prompt = user_prompt_sort_issues.format(
            all_issues=all_issues_formatted
        )
        ...
        sorted_indices_string = sorted_issue_indices[
        ...
        for index in sorted_indices_string:
            try:
                ...
                logger.error(
                    f"Failure parsing indices in sort_code_issues_by_severity: {e}"
                )
...
def group_vote_review_pr(
    ...
    for code_review_by_group in code_reviews_by_group:
        ...
        for group_name, code_review in code_review_by_group.items():
            ...
            prepped_code_review[group_name] = [
    ...
    for group_name, labels_dict in groups_to_labels_indexes.items():
        ...
        final_code_review: CodeReview = copy.deepcopy(
            code_reviews_by_group[0][group_name]
        )
...
def review_pr_detailed_checks(
    ...
    newly_created_functions_dict: dict[
        str, list[FunctionDef]
    ] = review_bot.identify_functions_in_patches(pr_changes, chat_logger=chat_logger)
    new_code_issues: dict[
        str, list[CodeReviewIssue]
...
def get_pr_summary_from_patches(
    pr_changes: dict[str, PRChange], chat_logger: ChatLogger | None = None
):
    review_bot = PRReviewBot()
...
def decompose_code_review_by_group(code_reviews_by_group: dict[str, CodeReviewByGroup]):
    ...
    for group_name, code_review_by_group in code_reviews_by_group.items():
        ...
        for file_name in file_names:
            issues = [
                issue
                for issue in code_review_by_group.issues
                if issue.file_name == file_name
            ]
            potential_issues = [
                issue
                for issue in code_review_by_group.potential_issues
                if issue.file_name == file_name
            ]
