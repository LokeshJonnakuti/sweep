...
def save_messages_for_visualization(
    messages: list[Message], use_openai: bool, model_name: str
):
    ...
    for i, function_name in enumerate(function_names):
        if function_name in functions_to_unique_f_locals_string_getter:
            unique_f_locals = functions_to_unique_f_locals_string_getter[function_name](
                frames[i].frame.f_locals
            )
            ...
            if i == len(function_names) - 2:
                subfolder = os.path.join(
                    subfolder, f"{current_hour_minute_second}_{function_name}"
                )
                os.makedirs(subfolder, exist_ok=True)
...
"""
        ...
        for i, message in enumerate(messages):
            try:
                ...
                f_html.write(
                    f"<details><summary>{html.escape(message_header)}</summary>\n<div>{content_html}</div>\n</details>\n\n"
                )
            except Exception as e:
                logger.error(f"Error processing message: {e}")
                f_raw.write(
                    f"Error in message processing: {e}\nRaw content: {content_raw}\n\n"
                )
                f_html.write(
                    f"<details><summary>Error in message processing</summary>\n<div><pre>{html.escape(str(e))}\n{html.escape(content_raw)}</pre></div>\n</details>\n\n"
                )
    ...
    cwd = os.getcwd()
    logger.info(
        f"Messages saved to {os.path.join(cwd, raw_file)} and {os.path.join(cwd, html_file)}"
    )
