...
class AnnotateCodeBot(ChatGPT):
    def annotate_code(
        ...
        try:
            ...
            return ""
        code_annotation_matches = re.search(
            annotate_code_pattern, code_annotation_response, re.DOTALL
        )
...
def process_chunk(idx, code_content, source_code, issue_text, file_path):
    ...
    formatted_code_content = (
        f'<original_code file_path="{file_path}" index="{idx}">\n'
        + code_content
        + "\n</original_code>\n"
    )
    formatted_annotation = (
        f'<code_summary file_path="{file_path}" index="{idx}">\n'
        + annotation
        + "\n</code_summary>\n"
    )
...
def get_annotated_source_code(source_code: str, issue_text: str, file_path: str):
    ...
    if NUM_WORKERS > 1:
        ...
        results = [
            pool.apply_async(
                process_chunk,
                args=(idx, code_content, source_code, issue_text, file_path),
            )
        ...
        for result in results:
            ...
            if chunk_result is not None:
                ...
                annotated_source_code = annotated_source_code.replace(
                    code_contents[idx], code_with_summary, 1
                )
        ...
        for idx, code_content in enumerate(tqdm(code_contents)):
            ...
            code_with_summary = f"{formatted_code_content + formatted_annotation}"
            annotated_source_code = annotated_source_code.replace(
                code_content, code_with_summary, 1
            )
            code_with_summaries.append(code_with_summary)
