...
def snippet_majority_vote(outcomes: list[list[Snippet]]):
    ...
    majority_snippets = [
        snippet
        for snippet, count in snippet_counts.items()
...
class AnalyzeSnippetAgent(ChatGPT):
    @majority_vote_decorator(
        num_samples=2, voting_func=snippet_majority_vote
    )  # unsure about 3 vs 1
    def analyze_snippets(
        self, snippets: list[Snippet], type_name: str, issue: str, seed: int = 0
    ):
        ...
        system_prompt = analyze_system_prompt.format(
            issue=issue, type_name=type_name, explanation=type_to_explanation[type_name]
        )
        self.messages = [Message(role="system", content=system_prompt)]
        user_prompt = analyze_user_prompt.format(
            issue=issue,
            type_name=type_name,
            explanation=type_to_explanation[type_name],
        ...
        relevant_snippets = re.findall(
            relevant_snippet_pattern, analyze_response, re.DOTALL
        )
        relevant_snippets = set(snippet.strip() for snippet in relevant_snippets)
        remaining_snippets = [
            snippet for snippet in snippets if snippet.file_path in relevant_snippets
        ]
