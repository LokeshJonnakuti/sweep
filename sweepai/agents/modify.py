...
def generate_code_suggestions(
    ...
    current_fcr_index = next(
        (i for i, fcr in enumerate(fcrs) if not fcr.is_completed), -1
    )
            ...
            if i < current_fcr_index:
                ...
                code_suggestions.append(
                    StatefulCodeSuggestion(
                        ...
                        original_code=parsed_fcr["original_code"][0]
                        if parsed_fcr["original_code"]
                        else "",
                        new_code=parsed_fcr["new_code"][0]
                        if parsed_fcr["new_code"]
                        else "",
...
def modify(
    ...
    llm_state = {
        ...
        "previous_attempt": "",
        "changes_per_fcr": [
            get_replaces_per_fcr(fcr) for fcr in fcrs
        ],  # how many old/new code pairs there are per fcr
        "completed_changes_per_fcr": [
            0 for _ in fcrs
        ],  # how many successful changes have been applied per fcr
    ...
    try:
        if compiled_fcr := compile_fcr(fcrs[0], 0):
            ...
            chat_gpt.messages.append(
                Message(  # this will happen no matter what
                    role="assistant", content=function_calls_string
                )
            )
    ...
    for i in range(len(fcrs) * 15):
        ...
        if function_call:
            ...
            if function_output == "DONE":
                ...
                break
            # breakpoint()
            detailed_chat_logger_messages.append(
            ...
            if modify_files_dict:  # update the state of the LLM
                ...
                if (
                    ...
                    llm_state["user_message_index_chat_logger"] = (
                        len(detailed_chat_logger_messages) - 1
                    )
                previous_modify_files_dict = copy.deepcopy(modify_files_dict)
        else:
        ...
        try:
            ...
            change_in_fcr_index = llm_state["completed_changes_per_fcr"][
                current_fcr_index
            ]
            ...
            if change_in_fcr_index >= max_changes:
                ...
                if llm_state["attempt_lazy_change"]:
                    if compiled_fcr := compile_fcr(
                        ...
                        # update messages to make it seem as if it called the fcr
                        chat_gpt.messages.append(
                            Message(role="assistant", content=function_calls_string)
                        )
                ...
                if not function_calls_string:
                    if linter_warning_prompt in function_output:
                        llm_state[
                            "attempt_count"
                        ] = 3  # skip to opus if there is a linter warning
    ...
    diff_string = ""
    for file_name, file_data in modify_files_dict.items():
        if diff := generate_diff(file_data["original_contents"], file_data["contents"]):
            diff_string += f"\nChanges made to {file_name}:\n{diff}"
    logger.info(
        "\n".join(
