...
class Tool:
    ...
    def get_xml(
        self, include_function_call_tags: bool = True, include_description: bool = True
    ):
        parameters_xml = "\n".join(
...
def parse_function_calls(
    response_contents: str, tools: list[Tool]
) -> list[dict[str, str]]:
...
def handle_function_call(
    function_call: AnthropicFunctionCall, tools: list[Tool], **kwargs
):
    for tool in tools:
        if function_call.function_name == tool.name:
            ...
            param_names = tool.function.__code__.co_varnames[
        ...
        return "ERROR\n\nInvalid tool name. Must be one of the following: " + ", ".join(
...
def get_function_call(
    ...
    **kwargs,
    ...
    function_call = validate_and_parse_function_call(response, chat_gpt, tools)
    ...
    if function_call is None:
        ...
        function_call_response = handle_function_call(function_call, tools, **kwargs)
        user_message = (
