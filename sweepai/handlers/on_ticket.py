...
from sweepai.config.client import RESET_FILE, REVERT_CHANGED_FILES_TITLE, SweepConfig
from sweepai.config.server import ENV, GITHUB_LABEL_NAME, IS_SELF_HOSTED, MONGODB_URI
...
from sweepai.utils.image_utils import (
...
from sweepai.utils.ticket_utils import fetch_relevant_files
...
def on_ticket(
    ...
    with logger.contextualize(
        ...
        ) = process_summary(summary, issue_number, repo_full_name, installation_id)
        ...
        try:
            ...
            if os.listdir(cloned_repo.repo_dir) == []:
                handle_empty_repository(
                    comment_id, current_issue, progress_headers, issue_comment
                )
            ...
            try:
                ...
                image_urls = get_image_urls_from_issue(
                    issue_number, repo_full_name, installation_id
                )
                ...
                if image_contents:  # doing it here to avoid editing the original issue
                    internal_message_summary += ImageDescriptionBot().describe_images(
                        text=title + internal_message_summary, images=image_contents
                    )
                ...
                for message, repo_context_manager in fetch_relevant_files.stream(
                    ...
                    if (
                        ...
                        edit_sweep_comment(message, 1, step_complete=False)
            ...
            try:
                ...
                for (
                    ...
                    edit_sweep_comment(
                        user_facing_message + planning_markdown, 2, step_complete=False
                    )
