...
@file_cache()
def download_logs(
    repo_full_name: str, run_id: int, installation_id: int, get_errors_only=True
):
...
def get_failing_docker_logs(cloned_repo: ClonedRepo):
    ...
    def run_dockerfile_config(
        dockerfile_config: DockerfileConfig, cloned_repo: ClonedRepo
    ):
        image_name = (
            dockerfile_config.image_name + "-" + str(hash(cloned_repo.repo_dir))[-8:]
        )
        container_name = (
            dockerfile_config.container_name
            + "-"
            + str(hash(cloned_repo.repo_dir))[-8:]
        )
        dockerfile_path = dockerfile_config.dockerfile_path
        env_path = os.path.join(
            os.path.join(os.getcwd(), os.path.dirname(dockerfile_path)), ".env"
        )
                ...
                with change_dir(cloned_repo.repo_dir):
                    ...
                    with Timer():
                        ...
                        with safe_command.run(
                            ...
                            for line in proc.stdout:
                                ...
                                yield status
                    status["message"] = (
                        f"Checks passed" if proc.returncode == 0 else f"Checks failed"
                    )
                    ...
                    with Timer():
                        ...
                        if proc.returncode == 0:
                            logger.info(
                                f"Docker container {container_name} exited successfully"
                            )
                            ...
                            logger.error(
                                f"Docker container {container_name} exited with error code {proc.returncode}"
                            )
    ...
    for i, dockerfile_config in enumerate(dockerfile_configs):
        ...
        if status["succeeded"] == False:
            statuses[i + 1 :] = [
                {**status, "status": "cancelled", "message": "Check cancelled"}
                for status in statuses[i + 1 :]
            ]
            yield statuses
...
def clean_cicd_logs(command: str, error: str, logs: str):
    ...
    if len(cleaned_logs) > MAX_LINES:
        ...
        cleaned_logs = (
            cleaned_logs[:LINES_TO_KEEP] + ["..."] + cleaned_logs[-LINES_TO_KEEP:]
        )
    cleaned_logs_str = "\n".join(cleaned_logs)
    error_content = ""
    if len(error) < 200000:
...
def get_circleci_job_details(job_number, project_slug, vcs_type="github"):
    ...
    url = (
        f"https://circleci.com/api/v1.1/project/{vcs_type}/{project_slug}/{job_number}"
    )
    ...
    for step in failing_steps:
        ...
        for action in actions:
            ...
            if "output_url" in action:
                ...
                log_message = (
                    log_response[0]["message"] if len(log_response) > 0 else ""
                )
                error_message = (
                    log_response[1].get("message", "") if len(log_response) > 1 else ""
                )
                log_message = remove_ansi_tags(log_message)
                error_message = remove_ansi_tags(error_message)
                command = action.get(
                    "bash_command", "No command found."
                )  # seems like this is the only command
...
def get_failing_circleci_logs(
    ...
    while True:
        ...
        if all(status.state == "success" for status in all_statuses):
            failing_statuses = []
            logger.debug(
                f"Exiting polling for CircleCI as all statuses are success. Statuses were: {all_statuses}"
            )
            break
        # if any of the statuses are failure, return those statuses
        failing_statuses = [
            status for status in all_statuses if status.state == "failure"
        ]
        if failing_statuses:
            logger.debug(
                f"Exiting polling for CircleCI as some statuses are failing. Statuses were: {all_statuses}"
            )
            break
        ...
        if any(status.state == "pending" for status in all_statuses):
            ...
            logger.debug(
                f"Polling to see if CircleCI has finished... {total_poll_attempts}."
            )
    ...
    if allowed_cicd_names:
        failing_statuses = [
            ...
            if any(
                cicd_name in status.context.lower() for cicd_name in allowed_cicd_names
            )
        ]
    # done polling
