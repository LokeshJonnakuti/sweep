...
from sweepai.core.pull_request_bot import (
    GHA_SUMMARY_END,
    GHA_SUMMARY_START,
    PRSummaryBot,
)
from sweepai.core.sweep_bot import (
    GHA_PROMPT,
    GHA_PROMPT_WITH_HISTORY,
...
from sweepai.handlers.on_check_suite import (
...
def get_error_locations_from_error_logs(error_logs: str, cloned_repo: ClonedRepo):
    ...
    try:
        ...
        for match in matches:
            ...
            errors[actual_file_path][
                int(line_number)
            ] = current_error_message  # assume one error per line for now
        ...
        for file_path, errors_dict in errors.items():
            ...
            for line_number in errors_dict.keys():
                erroring_lines |= set(
                    range(
                        line_number - surrounding_lines, line_number + surrounding_lines
                    )
                )
...
def handle_failing_github_actions(
    ...
    failing_logs, _ = get_error_locations_from_error_logs(
        failing_logs, cloned_repo=cloned_repo
    )
    diffs = get_branch_diff_text(
        repo=repo, branch=pull_request.head.ref, base_branch=pull_request.base.ref
    )
    ...
    snippets: list[Snippet] = prep_snippets(
        cloned_repo=cloned_repo, query=all_information_prompt, ticket_progress=None
    )  # need to do this, can use the old query for speed
    ...
    try:
        ...
        commit = commit_multi_file_changes(
            cloned_repo, new_file_contents_to_commit, commit_message, cloned_repo.branch
        )
        return commit
    except Exception as e:
...
def on_failing_github_actions(
    ...
    _main_runs: list[WorkflowRun.WorkflowRun] = list(
        repo.get_workflow_runs(
            branch=repo.default_branch, head_sha=pull_request.base.sha
        )
    )
    main_passing = True
    ...
    try:
        ...
        while GITHUB_ACTIONS_ENABLED and main_passing:
            ...
            current_commit = repo.get_pull(
                pull_request.number
            ).head.sha  # IMPORTANT: resync PR otherwise you'll fetch old GHA runs
            ...
            if DOCKER_ENABLED:
                branch = pull_request.head.ref
                failing_logs, new_docker_image_names = get_failing_docker_logs(
            ...
            if (
                ...
                and not failing_logs
            ):  # don't break if CircleCI or custom Dockerfile had failures
            ...
            if (
                not any([run.conclusion == "failure" for run in runs])
                and not failing_logs
            ):
                continue
            ...
            if failing_logs:
                ...
                try:
                    ...
                    _commit = commit_multi_file_changes(
                        cloned_repo,
                        new_file_contents_to_commit,
                        commit_message,
                        cloned_repo.branch,
                ...
                if total_edit_attempts >= GHA_MAX_EDIT_ATTEMPTS:
                    logger.info(
                        f"Tried to edit PR {GHA_MAX_EDIT_ATTEMPTS} times, giving up."
                    )
                    break
    finally:
    ...
    if CIRCLE_CI_PAT:
        try:
            ...
            failed_circleci_logs = get_failing_circleci_logs(
                repo=repo, current_commit=current_commit
            )
        except Exception as e:
